...
## 24-Hour EV Charging Demand Profile Generation and Forecasting for Sri Lanka (2025–2035)
**Researcher:** Herath H M K M — IT23249684
**Method:** Monte Carlo Simulation + LSTM Forecasting (Option B)
...
| # | Objective |
|---|---|
| O1 | Classify all candidate EV charging locations in Sri Lanka's 25 districts into four functional place typologies using OSM land-use data |
...
| O3 | Build a parameterized Monte Carlo simulation engine to generate annual 24-hour demand profiles for each district × typology × scenario combination (2025–2035) |
| O4 | Train LSTM forecasting models on simulated annual profiles to predict demand evolution for 2032–2035 |
...
| Year | `y` | Integer (2025–2035) | Simulation loop | Which projection year |
...
### 4.4 — LSTM Model Variables

| Variable | Description |
|---|---|
| Training input sequence | 7 annual per-EV normalized 24-hour profiles (2025–2031) per typology |
| Training target | Next year's per-EV normalized 24-hour profile |
| Test input sequence | 2025–2031 profiles (same as training context) |
| Test target | 2032–2035 profiles (held out, generated by Monte Carlo but never seen by LSTM) |
| LSTM hidden state | 64-dimensional learned representation |
| Forecast output | Predicted per-EV 24-hour profiles for 2032–2035 |
| Scaled forecast | Forecast × N_EVs(d,t,y,s) for each district |
...
|---|---|---|
| District EV count by year and scenario | Table: 25 rows × 11 years × 3 scenarios | Annual projected EV registrations per district for 2025–2035 under Low/Medium/High penetration |

> **Note:** If Component 1 outputs are not yet available, use placeholder growth rates (Low=5%, Medium=12%, High=20% annual from 2025 base count from DMT) until final values are received.
...
|---|---|---|---|
| T1 Urban Commercial | EV office commuters in Colombo / Kandy | Facebook "EV Sri Lanka", LinkedIn | 10–15 |
| T2 Suburban Residential | EV home-chargers in Gampaha / Moratuwa | Facebook EV groups, BYD / Nissan / MG dealerships | 10–15 |
| T3 Tourist Destination | Long-trip drivers (Colombo→Kandy/Galle/Nuwara Eliya) | Facebook road trip posts, Instagram EV community | 8–12 |
| T4 Highway Corridor | Long-distance commuters, hotel fleet EV operators | EV Facebook group, hotel HR contacts | 5–10 |
| **Total** | | | **33–52** |
...
For N in [100, 250, 500, 1000, 2500, 5000, 10000, 25000]:
    Run simulation N times for reference case (Kandy, T3, Medium, 2025)
...
```
11 years × 25 districts × 4 typologies × 3 scenarios = 3,300 runs
...
## 9. LSTM Forecasting Design

### Training Data Structure
```
Per typology:
  Training: 2025–2031 (7 years × 25 districts = 175 per-EV normalized profiles)
  + Data augmentation (×3 with Gaussian noise σ=2% amplitude) = 525 sequences
  Test (held out): 2032–2035 (4 years × 25 districts = 100 sequences)
```

### LSTM Architecture
| Hyperparameter | Value |
|---|---|
| Input size | 24 (one 24-hour profile) |
| Hidden size | 64 units |
| Layers | 2 stacked LSTM layers |
| Dropout | 0.2 |
| Output size | 24 (next year's profile) |
| Loss | MSE |
...
### Validation Protocol
```
LSTM trained on 2025–2031 profiles (augmented)
Predicts 2032–2035 without having seen those years
Compare predicted vs Monte Carlo 2032–2035 ground truth
...
```
Baseline 1: Linear interpolation between 2025 and 2035 MC profiles
Baseline 2: ARIMA(1,1,0) on peak demand time series
If LSTM < both baselines: use best baseline and report honestly
```
...
| `lstm_forecasts.csv` | District, Typology, Year (2032–2035), Predicted_H00–H23, Actual_H00–H23, RMSE, MAPE |
...
| SciPy | Distribution fitting (Beta, LogNormal, GMM) |
| Pandas | Data management and CSV I/O |
| OSMnx | OSM land-use polygon extraction |
...
| Jupyter Notebook | Development and results presentation |

---

## 14. Execution Timeline — 15 Weeks
...
| 10–11 | Monte Carlo Full Run | Run all 3,300 combinations. Save to monte_carlo_profiles.csv. Estimated: ~56 minutes. |
| 12 | LSTM Preparation | Extract per-EV normalized profiles. Prepare training + test sequences. Apply data augmentation. |
| 13 | LSTM Training | Train LSTM and ARIMA baseline. Evaluate on held-out 2032–2035 test set. Record RMSE/MAPE per typology. |
...
| Monte Carlo to LSTM Cold-Start Forecasting | First study training LSTM on synthetic Monte Carlo profiles to forecast demand evolution without historical charging data |
...
| LSTM may not outperform baselines | Planned comparison reported honestly — baseline used if LSTM fails |
