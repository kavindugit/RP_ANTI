...
|---|---|
| Target Series | 7 annual per-EV normalized 24-hour profiles (2025–2031) per typology |
...
```
Dataset Base: 2025–2031 (7 years × 25 districts = 175 per-EV normalized profiles per typology)
Data Augmentation (Mandatory to prevent overfitting): 
  + Gaussian noise injection & Time Jittering (×10 augmentation) 
  = 1,750+ training sequences per typology
...
### Validation Protocol
```
N-BEATSx trained on augmented 2025–2031 profiles
...
| NeuralForecast (Nixtla) / PyTorch | N-BEATSx model training |
